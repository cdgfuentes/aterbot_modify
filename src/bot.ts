import mineflayer, { Bot } from 'mineflayer'; // Change import statement for Mineflayer
import { sleep, getRandom } from './utils.ts';
import CONFIG from '../config.json' assert { type: 'json' };

let loop: NodeJS.Timeout;
let bot: Bot; // Change variable type to Bot
  ...
  try {
    ...
    bot.once('spawn', () => {
      const changePos = async (): Promise<void> => {
        const lastAction = getRandom(CONFIG.action.commands) as mineflayer.ControlState; // Adjust the type
